...
function isLikelyWashingMachine(row) {
  const text = `${row.product_name || ""} ${row.merchant_category || ""} ${row.category_name || ""}`.toLowerCase();
  const include =
    /(lavadora|washing machine|washer|lavasecadora|lava[-\s]?secadora)/i.test(text);
  if (!include) return false;
  const exclude =
    /(secadora|\btv\b|televisor|monitor|detergente|funda|manguera|accesorio|repuesto|pieza|escobilla|limpiador|kit|soporte)/i.test(text);
  return !exclude;
}
...
async function main() {
  ...
  const candidates = rows.filter(isLikelyWashingMachine);
  if (!candidates.length) throw new Error("No hay candidatos de lavadora en el feed.");
  ...
  try {
    ...
    for (const row of candidates) {
      ...
      const slugBase =
        slugify(`${name} ${model} ${row.aw_product_id || ""}`) || slugify(name) || `lavadora-fnac-${Date.now()}`;
      let slug = slugBase;
      let attempt = 1;
      ...
      await client.query(
        ...
        `,
        [productId, slug, name, "LAVADORAS", brand, model || "N/A", image, images, description, null, null, now, now]
      );
    ...
    if (inserted < TARGET_COUNT) {
      throw new Error(`Solo encontré ${inserted} lavadoras nuevas (objetivo ${TARGET_COUNT}).`);
    }
