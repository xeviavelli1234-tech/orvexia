...
function isLikelyDryer(row) {
  const name = String(row.product_name || "").toLowerCase();
  const text = `${row.product_name || ""} ${row.merchant_category || ""} ${row.category_name || ""}`.toLowerCase();
  const hasSecadoraWord = /\bsecadora\b/.test(name);
  const hasApplianceHints = /(bomba\s*de\s*calor|condensaci[oó]n|carga\s*frontal|carga\s*superior|\d+\s*kg|\d+\s*rpm)/i.test(
    text
  );
  if (!hasSecadoraWord || !hasApplianceHints) return false;
  const exclude =
    /(\bsecador\b(\s+de\s+pelo)?|hair\s*dryer|tendedero|dryerballs|bola(s)?\s+para\s+secadora|manguera|soporte|kit\s+uni[oó]n|accesorio|repuesto|pieza|limpiador|lavasecadora|lava[-\s]?secadora|lavadora\s+secadora|port[aá]til|eléctrica\s+port[aá]til|centrifugadora|desecadora)/i.test(
      text
    );
  return !exclude;
}
...
async function main() {
  ...
  const candidates = rows.filter(isLikelyDryer);
  if (!candidates.length) throw new Error("No hay candidatos de secadora en el feed.");
  ...
  try {
    ...
    for (const row of candidates) {
      ...
      const externalUrlKey = externalUrl.toLowerCase();
      if (seenUrl.has(externalUrlKey)) continue;
      ...
      const slugBase =
        slugify(`${name} ${model} ${row.aw_product_id || ""}`) || slugify(name) || `secadora-fnac-${Date.now()}`;
      let slug = slugBase;
      let attempt = 1;
      ...
      await client.query(
        ...
        `,
        [productId, slug, name, "SECADORAS", brand, model || "N/A", image, images, description, null, null, now, now]
      );
      ...
      seenUrl.add(externalUrlKey);
      inserted += 1;
      logs.push(`${inserted}. ${name} | ${priceCurrent.toFixed(2)}€`);
    }

    if (inserted < TARGET_COUNT) {
      throw new Error(`Solo encontré ${inserted} secadoras nuevas (objetivo ${TARGET_COUNT}).`);
    }

    await client.query("COMMIT");
    console.log(JSON.stringify({ ok: true, inserted, scanned }, null, 2));
