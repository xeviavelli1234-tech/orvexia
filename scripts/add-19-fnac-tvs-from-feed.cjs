...
function isLikelyTv(row) {
  const name = `${row.product_name || ""}`.toLowerCase();
  const text = `${row.product_name || ""} ${row.merchant_category || ""} ${row.category_name || ""}`.toLowerCase();
  const tvHints =
    /(televisor|smart\s*tv|google\s*tv|android\s*tv|\btv\b|qled|oled|uhd|4k|8k|mini\s*led|bravia)/i.test(name);
  if (!tvHints) return false;
  if (
    /(proyector|monitor|soporte|funda|cable|antena|cuaderno|libro|ebook|rotulador|notebook|port[aá]til|l[aá]mpara|marcador|tote bag|paperblanks|bolsillo interior|set colores)/i.test(
      text
    )
  ) {
    return false;
...
async function main() {
  ...
  try {
    ...
    const seenNameModel = new Set(
      existing.rows.map((r) => `${r.name_key}::${r.model_key}`)
    );
    ...
    for (const row of candidates) {
      ...
      const slugBase = slugify(`${name} ${model} ${row.aw_product_id || ""}`) || slugify(name) || `tv-fnac-${Date.now()}`;
      let slug = slugBase;
      let attempt = 1;
      ...
      while (
        (
          // eslint-disable-next-line no-await-in-loop
          await client.query('select 1 from "Product" where slug = $1 limit 1', [slug])
        ).rowCount > 0
      ) {
        attempt += 1;
    ...
    if (inserted < TARGET_COUNT) {
      throw new Error(`Solo encontré ${inserted} candidatos nuevos (objetivo ${TARGET_COUNT}).`);
    }
