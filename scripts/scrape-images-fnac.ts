/**
 * scrape-images-fnac.ts
 *
 * Enriquece productos de Fnac con la galería completa de imágenes.
 * El feed AWIN solo expone `large_image` + `alternate_image` (1-2 imgs),
 * así que vamos a la página del producto y extraemos todas las imágenes
 * desde JSON-LD, OpenGraph y el DOM.
 *
 * Uso:
 *   npx tsx scripts/scrape-images-fnac.ts
 *   npx tsx scripts/scrape-images-fnac.ts --dry-run
 *   npx tsx scripts/scrape-images-fnac.ts --slug fnac-12345-tv-led-32-sony-kd32w800
 *   npx tsx scripts/scrape-images-fnac.ts --only-few    # solo productos con ≤1 imagen
 *   npx tsx scripts/scrape-images-fnac.ts --limit 20
 */
...
function normalizeFnacImage(raw: string): string | null {
  const url = raw
    .replace(/\\u002F/g, "/")
    .replace(/\\\//g, "/")
    .replace(/&amp;/g, "&")
  ...
  // Aceptamos imágenes del CDN de Fnac (static.fnac-static.com y similares)
  if (!/fnac-static\.com|fnac\.es|fnac\.com/i.test(url)) return null;
  if (!/\.(jpg|jpeg|png|webp)$/i.test(url)) return null;
  // Filtra miniaturas/iconos diminutos (evitamos logos de pago, sellos…)
...
function extractImages(html: string): string[] {
  ...
  const push = (raw: string) => {
    const n = normalizeFnacImage(raw);
    if (n && !seen.has(n)) {
      seen.add(n);
      out.push(n);
    }
  ...
  // 3) <img src/data-src/data-zoom-image> apuntando al CDN de Fnac
  for (const m of html.matchAll(
    /<img[^>]+(?:data-src|data-zoom-image|data-original|src)="([^"]*fnac-static\.com[^"]*\.(?:jpg|jpeg|png|webp))"/gi,
  )) {
    push(m[1]);
  }

  // 4) URLs escapadas en bloques JS inline
  for (const m of html.matchAll(
    /https?:\\\/\\\/[^"'\s\\]*fnac-static\.com[^"'\s\\]*\.(?:jpg|jpeg|png|webp)/gi,
  )) {
    push(m[0]);
  }

  // 5) URLs sueltas en comillas
  for (const m of html.matchAll(
    /["'`](https?:\/\/[^"'`\s]*fnac-static\.com[^"'`\s]+\.(?:jpg|jpeg|png|webp))["'`]/gi,
  )) {
    push(m[1]);
...
async function run() {
  console.log(
    `Scrape Fnac images${DRY_RUN ? " — DRY RUN" : ""}${
  ...
  const offers = await prisma.offer.findMany({
    where: {
      store: { contains: "fnac", mode: "insensitive" },
  ...
  if (pool.length === 0) {
    console.log("No hay ofertas de Fnac que procesar.");
    return;
  }
  ...
  for (const offer of pool) {
    ...
    try {
      ...
      if (!/fnac\.(es|com)/i.test(finalUrl)) {
        console.log(`  [skip] AWIN no resolvió a Fnac — ${label}`);
        failed++;
        await sleep(800);
        continue;
      }
