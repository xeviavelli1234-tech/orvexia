...
export function Header({ onSwap }: { onSwap?: () => void }) {
  return (
...
export function Row({ label, trophy, children }: { label: string; trophy?: "a" | "b" | null; children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-[64px_1fr_1fr] sm:grid-cols-[140px_1fr_1fr] border-t border-border-subtle">
...
export function Cell({ children, winner }: { children: React.ReactNode; winner?: boolean }) {
  return (
    <div
...
export function ScoreCell({ score, winner }: { score: number; winner: boolean }) {
...
export function SpecChips({ items }: { items: string[] }) {
...
export function ProsConsList({ items, tone }: { items: string[]; tone: "pro" | "con" }) {
  if (items.length === 0) return <span className="text-fg-subtle text-[11px]">—</span>;
...
export function OffersList({ offers }: { offers: DetailedOffer[] }) {
