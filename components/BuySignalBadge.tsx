...
export function BuySignalBadge({ productId, store }: { productId: string; store: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold" style={{ backgroundColor: c.bg, color: c.text }}>
...
function TrendArrow({ slope }: { slope: number | null }) {
  ...
  const text = up
    ...
    : down
    ? `Bajando ${Math.abs(slope).toFixed(2)} EUR/dia`
    : "Precio estable";

  return (
    <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${color}18`, color }}>
      {icon} {text}
    </span>
  );
}
...
export function BuySignalPanel({ productId, store }: { productId: string; store: string }) {
  ...
  const c = COLOR_MAP[data.color];
  const hasPrice = data.priceScore !== null && data.priceMin90d !== null && data.priceAvg90d !== null && data.priceMax90d !== null;
  const hasTrend = data.trendScore !== null && data.trendSlope !== null;

  const priceCtx = hasPrice
    ? data.priceScore! >= 32
      ? "Cerca del minimo historico"
      : data.priceScore! >= 20
      ? "En rango medio"
  ...
  const trendCtx = hasTrend
    ? data.trendSlope! > 0.1
      ? `Subiendo ${data.trendSlope!.toFixed(2)} EUR/dia`
      : data.trendSlope! < -0.1
      ? `Bajando ${Math.abs(data.trendSlope!).toFixed(2)} EUR/dia`
      : "Precio estable"
    : "";

  const storeCtx = data.storeScore >= 16 ? "Tienda con buen historial" : data.storeScore >= 10 ? "Historial mixto" : "Inconsistencias detectadas";

  const dealCtx = data.isFakeDeal
    ? "Descuento posiblemente inflado"
    : data.dealScore >= 12
    ? "Descuento bien respaldado"
    : data.dealScore >= 7
    ? "Descuento moderado"
    : "Sin descuento activo";

  let reconstructedCurrent: number | null = null;
  if (hasPrice) {
    const pct = 1 - (data.priceScore ?? 0) / 40;
    reconstructedCurrent = data.priceMin90d! + pct * (data.priceMax90d! - data.priceMin90d!);
  }
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: c.bg, color: c.text }}>
              ...
              {data.historyDays} dias de datos
            </span>
            {data.isEstimated && (
              <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#EEF2FF] text-[#4338CA]">
      ...
      {open && (
        <div className="border-t border-[#F1F5F9] px-4 pb-4 space-y-4 pt-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <FactorCard icon="💰" label="Precio historico" score={data.priceScore} max={40} barColor={c.bar} context={priceCtx} noData={!hasPrice} estimated={data.estimatedPrice} />
            <FactorCard icon="📈" label="Tendencia" score={data.trendScore} max={25} barColor={c.bar} context={trendCtx} noData={!hasTrend} estimated={data.estimatedTrend} />
            <FactorCard icon="🏬" label="Fiabilidad tienda" score={data.storeScore} max={20} barColor={c.bar} context={storeCtx} noData={false} />
            <FactorCard icon="🏷️" label="Descuento real" score={data.dealScore} max={15} barColor={c.bar} context={dealCtx} noData={false} />
          </div>
          ...
          <div className="flex items-center justify-between rounded-xl px-4 py-3" style={{ backgroundColor: c.bg }}>
            <div>
              ...
              <p className="text-[10px]" style={{ color: c.text, opacity: 0.6 }}>
                {data.isEstimated
                  ? "Parte del analisis es orientativo hasta reunir mas historial"
                  : data.historyDays < 5
                  ? "Con mas historial mejorara el analisis"
                  : "Basado en 90 dias de datos reales"}
              </p>
            </div>
