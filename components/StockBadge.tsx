...
export function StockBadge({ inStock, store, discountPercent, productName }: Props) {
  ...
  if (
    ...
    return (
      <span className="inline-flex items-center gap-1.5 text-[11px] sm:text-xs font-semibold text-[#991B1B] bg-danger-50 border border-[#FECACA] px-2.5 py-1 rounded-full w-fit max-w-full break-words uppercase tracking-wide">
  ...
  if (discountPercent && discountPercent > 0) {
    return (
      <span className="inline-flex items-center gap-1.5 text-[11px] sm:text-xs font-medium text-hot-700 bg-amber-50 border border-[#FDE68A] px-2.5 py-1 rounded-full w-fit max-w-full break-words">
        <span className="w-2 h-2 rounded-full bg-warn-500 shrink-0" />
        {discountLabel}
      </span>
    );
  }
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-[11px] sm:text-xs font-medium text-accent-700 bg-accent-50 border border-accent-300 px-2.5 py-1 rounded-full w-fit max-w-full break-words">
