...
> = {
  DISCUSION: { label: "Discusión", bg: "var(--brand-50)", color: "var(--brand-600)", accent: "var(--brand-600)", dot: "var(--brand-600)" },
  PREGUNTA:  { label: "Pregunta",  bg: "#FEF3C7", color: "#B45309", accent: "var(--warn-500)", dot: "var(--warn-500)" },
  CHOLLO:    { label: "Chollo",    bg: "#DCFCE7", color: "#15803D", accent: "var(--accent-600)", dot: "var(--accent-600)" },
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
          ...
          <div className="space-y-5">
            ...
            <article className="bg-bg-elevated border border-border rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="space-y-3">
                  ...
                  <h1 className="text-2xl sm:text-3xl font-extrabold text-fg leading-tight">
                    {post.title}
                  </h1>
                </div>
                ...
                <div className="flex items-center gap-3 pb-4 border-b border-border-subtle">
                  ...
                  <div>
                    <p className="text-sm font-bold text-fg">{post.user.name}</p>
                ...
                {/* Content */}
                <div className="text-[15px] text-fg leading-relaxed whitespace-pre-line">
                  {post.content}
                </div>
                ...
                <div className="flex items-center gap-3 pt-2 border-t border-border-subtle">
                  {session ? (
                    ...
                    <div className="flex items-center gap-3">
                      <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-bg-subtle text-fg-subtle text-sm font-semibold">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                          <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        {initialCount}
                      </span>
                      <Link href="/login" className="text-xs text-brand-600 hover:underline font-medium">
                        Inicia sesión para votar
                      </Link>
                    </div>
                  )}
                  <span className="ml-auto text-xs text-fg-subtle flex items-center gap-1">
            ...
            {/* Comments */}
            <section>
              <h2 className="text-sm font-bold text-fg-muted uppercase tracking-widest mb-4">
                    ...
                    >
                      <div className="flex flex-col items-center gap-2 shrink-0">
                        ...
                        {i < post.comments.length - 1 && (
                          <div className="w-px flex-1 bg-bg-subtle min-h-[12px]" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0 pt-0.5">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="text-sm font-bold text-fg">{comment.user.name}</span>
                          <RelativeTime iso={comment.createdAt.toISOString()} className="text-[11px] text-fg-subtle" />
              ...
              {session ? (
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <Avatar
                      user={{
                        ...
                        avatarColor: currentUser?.avatarColor ?? "var(--brand-600)",
                        avatarEmoji: currentUser?.avatarEmoji ?? null,
                        avatarUrl: currentUser?.avatarUrl ?? null,
                      }}
                      size={32}
                    />
                    <p className="text-sm font-semibold text-fg">Añadir comentario</p>
                  </div>
                ...
                <div className="bg-bg-elevated border border-border rounded-2xl p-6 text-center">
                  ...
                  <p className="text-sm font-semibold text-fg mb-1">¿Tienes algo que aportar?</p>
                  <p className="text-xs text-fg-muted mb-4">Inicia sesión para unirte a la conversación.</p>
                  <Link
          ...
          <aside className="space-y-4">
            ...
            <div className="bg-bg-elevated border border-border rounded-2xl p-4 space-y-3">
              ...
              <div className="space-y-2.5">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-fg-muted">Votos</span>
                  <span className="font-bold text-fg">{initialCount}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-fg-muted">Comentarios</span>
                  <span className="font-bold text-fg">{post.comments.length}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-fg-muted">Publicado</span>
                  <span className="font-semibold text-fg-muted text-xs text-right max-w-[120px]">
