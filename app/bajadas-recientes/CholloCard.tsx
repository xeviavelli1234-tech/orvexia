          ...
          <svg key={i} className="w-3 h-3" viewBox="0 0 20 20">
            {partial ? (
              <>
                ...
                <path d="M10 1l2.39 4.84 5.34.78-3.87 3.77.91 5.32L10 13.27l-4.77 2.51.91-5.32L2.27 6.62l5.34-.78z" fill="var(--warn-500)" clipPath={`url(#ch-half-${i}-${rating})`} />
              </>
            ) : (
      ...
      >
        ...
        <div className="flex flex-col flex-1 p-5">
          <p className="text-xs mb-1">
            <span className="font-semibold text-hot-700">{product.brand}</span>
          </p>
          <h3 className="text-sm font-bold text-fg leading-snug line-clamp-2 mb-3 flex-1
                         group-hover:text-hot-700 transition-colors">
            {product.name}
          </h3>
          ...
          {product.rating != null && (
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold text-fg">{product.rating.toFixed(1)}</span>
              {product.reviewCount != null && (
                <span className="text-xs text-fg-subtle">({product.reviewCount.toLocaleString("es-ES")})</span>
              )}
            </div>
          )}
